...
To install a portable version of emscripten, you can execute the script "install_emscripten_portable.sh" (Windows users should use the file "install_emscripten_portable.cmd").
The script will create a folder named "emsdk"(size ~1.3GB) in the current working directory.
To setup the PATH variable you can execute "source WORKINGDIR/emsdk/emsdk_env.sh" or add this command to your .bashrc file. (Test: Execute "emcc" and you should get the error message "WARNING:root:no input files")
...
It is advisable to use the local webserver from emscripten to avoid some browser origin restrictions. To start the webserver with "prime_probe" as the root directory execute "WORKINGDIR/emsdk/emscripten/1.38.11/emrun --no_browser --port 8080 REPOS/code/wasm/prime_probe".
