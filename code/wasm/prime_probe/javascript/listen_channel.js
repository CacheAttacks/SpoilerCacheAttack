if (typeof Module['noisy_start_cache_set'] !== 'undefined') {
  ...
  console.log(
      'listen on ' +
      (Module['noisy_start_cache_set'] +
       COMMUNICATION_CHANNEL_OFFSET_START));  //+ " and " +
                                              //(Module['noisy_start_cache_set']
                                              //+ 32));
  ...
  d = new Date();
  Module['imgElement'].src = 'http://localhost:8000/plotchannel' +
      '?' + d.getTime();
  ...
  console.log(
      'Module[\'noisy_start_cache_set\'] is undefined! Searched for covert channel?');
}