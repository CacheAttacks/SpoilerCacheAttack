...
for (var bound = uint32ptrResults + nrecords; uint32ptrResults < bound;){
    uint32wasmMemory[++uint32ptrResults] = JSprobeTimeSplit2(uint32wasmMemory, uint8ptrMonitorhead, uint8ptrBmonitorhead);
    //break;

    // if (even){
    //     uint32wasmMemory[++uint32ptrResults] = JSprobeTime(uint32wasmMemory, uint8ptrMonitorhead);
    //     //localuint16[i] = JSprobeTime(uint32wasmMemory, uint8ptrMonitorhead);
    //     //uint32wasmMemory[++uint16ptrResults] = uint16ptrResults;
    // }
    // else{
    //     uint32wasmMemory[++uint32ptrResults] = JSprobeTime(uint32wasmMemory, uint8ptrBmonitorhead);
