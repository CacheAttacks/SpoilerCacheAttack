...
Beim Prime-and-Probe-Angriff steht die Browserimplementierung einer nativen Implementierung in nichts nach. Allerdings ist neben den Kosten für den Counter-Thread die zeitliche Auflösung des Angriffs um \todo{c code slot time messen} TODO \% reduziert.
...
Diese nutzt spezifische Eigenschaften der Store-Queue in Intel-Prozessoren aus, womit die Suche gegenüber dem optimierten Standardalgorithmus um etwa den Faktor 3 beschleunigt werden kann.
...
Bei der Leakage in der RSA-Primzahlgenerierung von Mozilla NSS und OpenPGP.js ist offen, ob sich diese eignet, Teile der Primzahlen \todo{Was ist damit gemeint?} effizient zu rekonstruieren.
...
Zudem werden Prime-and-Probe-Implementierungen in Javascript und Webassembly verglichen.
