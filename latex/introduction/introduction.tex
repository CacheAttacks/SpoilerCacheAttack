...
%(Es fehlt mir ein einleitender Satz wie etwa "Im Bereich der IT-Sicherheit gewinnt der externe Zugriff auf Daten Anderer zunehmende Bedeutung.") 
...
Ausführung von Programmcode %(bitte kurz erläutern!)
...
Eine Reihe von Ressourcen hat sich als problematisch erwiesen %(Warum?)
...
%\todo{Ich wäre hier für section Aufgabenstellung. Außerdem ein kleines bisschen blbla: In Dieser Arbeit wird der Angrif aus PAPER nachimplementiert. Es handelt sich dabei um einen P and P Angriff, der über TECHNOLOGIE durchgeführt wird. Durch TECHNOLOGIE bekommt man VORTIELE. Im Laufe der Implementierung werden folgende Fragen bearbeitet: }
...
\begin{enumerate}
\item Die Arbeiten \cite{TheSpyInTheSandbox,DriveByPaper,ASLROnTheLine} haben demonstriert, dass Cache-Angriffe im Browser möglich sind. %(besser als Frage formulieren) Hier als kurze Einführung zur Frage weiter unten gedacht?
...
Beim Prime-and-Probe-Angriff steht die Browserimplementation einer nativen Implementation in nichts nach. Allerdings ist neben den Kosten für den Counter-Thread die zeitliche Auflösung des Angriffs um \todo{c code slot time messen} \% reduziert.
...
Damit kann die Suche gegenüber dem optimierten Standardalgorithmus um \todo{Benchmarks für StoreFor vorantreiben} \% beschleunigt werden.
...
Hier soll ein Überblick bereits getätigter Forschungsarbeiten im Rahmen der für die Arbeit relevanten Themenbereiche %Cache-Angriffe 
%\todo{würde ich rausnehmen, cache-Angriffe sind ein zu großes Feld hier}, 
Drive-by-Attacks 
