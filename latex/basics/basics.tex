...
Heutige Prozessoren, wie etwa die Intel-Core-Reihe, führen auch (Warum "auch"? Das Wort würde ich streichen.) Store- und Load-Befehle Out-of-Order aus und stehen damit vor der Aufgabe, auf Datenabhängigkeiten (Was ist das denn?) zu reagieren \cite{memoryDisambiguationBlog}.
Die Techniken für eine Speicher-Disambiguierung erkennen echte Abhängigkeiten zwischen Speicheroperationen während der Ausführung. (Der Satz steht hier ohne Zusammenhang.)
...
Um dies (Bezug fehlt mir.) umzusetzen, werden Store-Instruktionen bei der Ausführung nicht direkt an das Speichersystem, in diesem Fall den CPU-Cache, committed (Also hier bitte auf deutsch: "Übermittelt"?)
...
Auch Write-after-Write(WAW)-Konflikte werden dadurch gelöst, da keine früheren STORES ihre Werte nach späteren STORES in das Speichersystem committen (s. o.), wie es ohne STORE-Queue bei der Out-Of-Order-Ausführung der Fall wäre. 
...
Diese Überlegung führt zu einem neuen Problem (bei ???), welches durch die Store-Queue entsteht.
Angenommen ein STORE wird ausgeführt und seine Adresse und seine Daten werden in der Store-Queue gepuffert. Kurz danach liest ein LOAD von der selben Adresse, auf die der STORE geschrieben hat.
Würde der LOAD den Wert vom Speichersystem lesen, wäre der Wert veraltet, da der im Code vorher stehende STORE noch nicht committed (s. o.) wurde.

Um diesem Problem zu begegnen, nutzten Prozessoren in der Store-Queue die store-to-load-forwarding-Technik, die die Store-Queue veranlasst,neben dem Puffern der STORES den Zweck Werte (mir unklare Substantive!!!) von in der Queue abgeschlossenen STORES, die noch nicht committed (s. o.) wurden, zu späteren LOADS weiterzuleiten.
...
Sofern es einen Treffer gibt, wird dieser Wert statt des veralteten Wertes aus dem (???) Speichersystem verwendet.
...
OpenPGP.js \cite{OpenPGPjs} ist eine Opensource-Implementierung des OpenPGP-Protokolls in Javascript, welche sich zum Ziel gesetzt hat, OpenPGP auf einer breiten Platte (meinst du "Palette"?) von Endgeräten zu ermöglichen.
