...
Die Idee ist nun im Thread des Webworkers eine Zählvariable des SharedArrayBuffer in einer Endlosschleife zu iterieren (siehe auch den Pseudocode \ref{alg:sharedArrayBufferWorker}). Zuerst wird dazu dem Webworker via message die Referenz auf einen im Mainthread erstellten SharedArrayBuffer übergeben. Anschließend wird im Webworker ein \textit{Uint32Array} angelegt welches mit dem übergebenen SharedArrayBuffer verknüpft ist. Zum Schluss geht der Webworker in die Endlosschleife über, in welcher die Zählvariable \textit{sharedArray[0]} durchgehend iteriert.
...
\begin{algorithm}[h]
\DontPrintSemicolon
\caption{Psuedo-Code für den Webworker welcher die Zählvariable iteriert}
\label{alg:sharedArrayBufferWorker}
