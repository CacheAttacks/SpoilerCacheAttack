...
Jedoch lässt sich die Eigenschaft ausnutzen, dass im Betriebssystem 4-KiB Pages existieren, sodass die letzten 12 Bit der virtuellen und physischen Adresse identisch sind. 
...
Danach wird die Zugriffszeit auf die Adresse gemessen und bei einer erhöhten Zeit aus dem Pool entfernt, da dann die Adresse auf das selbe Cache-Set wie die Einträge des Eviction-Set beziehungsweise die letzte Zeugenadresse abgebildet wird.
...
Die Forscher TODO sind auf ein Verhalten bei Intel-Prozessoren gestoßen, welches es ermöglicht Adressen zu finden deren 20 letzten physischen Bits gleich sind, ohne auf Techniken wie Huge-Pages zugreifen zu müssen.
Solche Adressen seinen folgend \textit{colliding addresses} genannt.
Die Idee ist den Store-Buffer in einer Schleife mit einer Vielzahl von Schreibbefehlen zu fluten und direkt danach eine Zeitmessung für das Lesen einer Adresse $x$ auszuführen.

Wenn einer der zuletzt hinzugefügten Schreibbefehle auf eine Adresse schreibt deren letzten 20 physischen Adressbits identisch mit denen der Read-Adresse $x$ sind, dann lässt sich ein Ausschlag bei der Leseoperation für $x$ feststellen.

Pseudocode \ref{alg:storeForward} beschreibt das Suchverfahren um colliding addresses zu finden.
Zuerst wird ein großer Speicherbereich alloziert, welcher ein vielfaches der Page-Größe von 4 KiB ist.
Wie im Abschnitt \ref{evictionSetSearchAlgo} wird hier mittels 4KiB-Speicherblöcken gesucht, da so die letzten 12 Bits der Adresse mit Sicherheit identisch sind.
...
Wie oben beschrieben muss der Store-Buffer mit Schreibbefehlen geflutet werden, wobei $WINDOW$ die Anzahl ebendieser angibt.
...
Der Testvorgang für einen Speicherblock $p$ wird mehrfach wiederholt (Zeile 5) und die Zugriffszeit auf $evictionBuffer[0]$ gemittelt (Zeile 11) um anders begründete Ausreißer bei der Zugriffszeit  auszuschließen. 

Grafik \ref{fig:colliding_addresses_js_measurement} zeigt das typische Verhalten der Zugriffszeiten auf $evictionBuffer[0]$, stellt also $measurementBuffer$ visuell dar.
...
Tests haben ergeben, dass mit $WINDOW=64$ colliding addresses zuverlässig durch Peaks bei der Zeitmessung der Leseopertion für $x$ hervorstechen.
Ein höher Wert für $WINDOW$ bringt keine Vorteile bei der Identifizierung der colliding addressses, erhöht aber die Laufzeit der Suche.
Andersherum verringern kleinere Werte für $WINDOW$ die Suchlaufzeit, aber sorgen auch für kleinere Peaks der colliding addresses bei der Zeitmessung, sodass eine Identifikation schwieriger wird.
Da Identifizierung unter Javascript bereits gegenüber C erschwert ist, existiert kein Spielraum um den Wert von $WINDOW$ zu abzusenken. TODO bench that 
