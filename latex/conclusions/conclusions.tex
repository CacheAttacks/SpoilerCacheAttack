...
In diesem Kapitel werden die wesentlichen Ergebnisse dieser Arbeit zusammengefasst und die gewonnenen Erkenntnisse bewertet (und im Hinblick auf die vier Ausgangsfragen Lösungsansätze präsentiert/beurteilt/überprüft).
...
ist dieser in der Praxis aufwendig, (Nein, nur ein Prädikat und damit ein Hauptsatz)
...
Eine Angreiferin (weiblich?) könnte versuchen, die Website zu manipulieren, welche die Zertifikatsgenerierung beschreibt.
...
Als mögliche Lösung könnte die Angreiferin (weiblich?) $n-1$ Instanzen ihres (weiblich?) Angriffs starten, wobei $n$ der Anzahl der virtuellen Kerne entspricht. 
...
Cache-Angriffe im Browser sind allgegenwärtig und ihnen sollte von Softwareseite mit den genannten Gegenmaßnahmen begegnet werden, da Websprachen der Angreiferin (weiblich?) die komplette Kontrolle über den Code überlassen und hardwareseitige Lösungen erst in einigen Jahren flächendeckend verfügbar sein werden.
